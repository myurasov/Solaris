...
projects/<slug>/                # container (not a git repo)
  ...
  <repo>/                        # THE repo (its own .git); e.g. "source"
    ...
    ai/                          # the ai-pack, embedded in the repo
      engineer.agent.md  engineer.instructions.md  spec.md  manifest.json  defaults.json  rules/  skills/  info/  .memory/  <plugin>/
...
`ai/skills/*.skill.md` stubs (`init`, `refresh`), the `ai/info/*.md` reference data, and the `ai/<plugin>/` overlays - portable, safe to share or hand off) and a **private/local
...
- **Subagents** (`rules/subagents.rule.md`; pack copy `ai/rules/subagents.rule.md`): delegate
  ...
  Abstract tiers (cheap/mid/high/frontier) map to concrete models in `solaris/info/model-tiers.md`
  (pack: `ai/info/model-tiers.md`, hard-required - never substituted from memory); a harness with no
  subagent tool (`solaris/info/harnesses.md`; pack: `ai/info/harnesses.md`) falls back to `off` behavior. Destructive,

...
projects/<slug>/
  ...
  ai/                           # shareable layer (agent + instructions + spec + rules/ + skills/ + info/ + defaults.json)
    README.md                   # generated pack overview + how-to (rev-marked; its {{PLUGINS}} list renders from the manifest)
...
`{{NAME}}`, `{{TYPE}}`, `{{MODE}}`, `{{DESCRIPTION}}`, `{{FRAMEWORK_VERSION}}`, `{{DATE}}`, and the
derived `{{PLUGINS}}` - the attached-plugins list rendered from the manifest into `ai/README.md`). Project types

# Solaris v0.3.0 - Specification <!-- omit in toc -->
...
Authoritative description of Solaris v0.3.0. Supersedes [`spec-v0.1.0.md`](spec-v0.1.0.md) and the 0.2.0
revision (in git history), alongside the original brief [`spec-v0.txt`](spec-v0.txt) and the v0.1.0 build
plan [`plan-v0.1.0.md`](plan-v0.1.0.md). What changed in 0.3.0: `developer.instructions.md` moves out of
`ai/memory/` up to `ai/` - the shareable, portable layer alongside `developer.agent.md` and `spec.md` -
leaving `ai/memory/` as the private/local layer (resources, credentials, logs); see
[`../migrations/0.3.0.md`](../migrations/0.3.0.md). What changed in 0.2.0: per-file **revisions** drive
ai-setup sync (versions become release-only), `CLAUDE.md` and `.cursor/rules` shims are gone (both IDEs read
`AGENTS.md`), and project roots are trimmed to just `AGENTS.md`.
...
projects/<slug>/
  ...
  ai/                           # shareable layer (developer.agent.md + developer.instructions.md + spec.md)
    developer.agent.md          # combined coder + planner + runner (carries a rev marker)
    developer.instructions.md   # shareable build/run/test commands + conventions (no host/secret specifics)
...
The ai-setup splits into a **shareable layer** (`ai/developer.agent.md`, `ai/developer.instructions.md`,
`ai/spec.md`, and the `ai/<plugin>/` overlays - portable, safe to share or hand off) and a **private/local
layer** (`ai/memory/`: hosts, secrets, internal URLs, the preserved spec, logs). To share an ai-setup, drop
`ai/memory/`. It is materialized from `solaris/templates/ai-setup/` with placeholder substitution (`{{SLUG}}`,
...
level up in `ai/developer.instructions.md`, and any host/secret/internal-URL detail that surfaces there is
relocated down into `ai/memory/` rather than dropped. A prompt-submit hook (`log_interaction`) appends one
JSON line per turn, routed by cwd (inside `projects/<slug>/` -> that project's log, else framework). It is
fail-safe and unbounded in v0.

# Solaris v0.33.0 - Specification <!-- omit in toc -->
...
project, the **primary persona** (`projects/<slug>/ai/<primary>.agent.md` - the `engineer` role unless the
manifest's `agents.primary` renames it) loads the ai-pack, every `ai/plugins/<plugin>/` overlay, and
`source/AGENTS.md` if present; optional **role personas** (`ai/agents/<role>.agent.md` briefs) are used by
telling a model to act as that file. "Hand off" = switching the active instruction set + working directory.
...
projects/<slug>/
  ...
  ai/                           # shareable layer (primary persona + instructions + spec + agents/ + rules/ + skills/ + info/ + defaults.json)
    ...
    <primary>.instructions.md  # shareable build/run/test commands + conventions (no host/secret specifics)
    agents/                     # optional role personas: <role>.agent.md briefs (frontmatter description/tier/access + the brief; project content, no rev marker)
...
projects/<slug>/                # container (not a git repo)
  ...
  <repo>/                        # THE repo (its own .git); e.g. "source"
    ...
    ai/                          # the ai-pack, embedded in the repo
      README.md  <primary>.agent.md  <primary>.instructions.md  spec.md  manifest.json  defaults.json  agents/  rules/  skills/  info/  .memory/  plugins/<plugin>/
...
**Personas.** Every pack has one **primary persona** - `ai/<primary>.agent.md` (materialized from the
template `engineer.agent.md`) plus `ai/<primary>.instructions.md` - named `engineer` unless the manifest's
`agents.primary` renames it (`uv run -m solaris.tools.agents --rename-primary <role> --dir <project>` moves
the two files, sets the key, and re-renders the managed files; the `{{PRIMARY}}` / `{{PRIMARY_TITLE}}`
placeholders carry the name into every rendered file, so the docs' `engineer.*` names mean "the primary").
Additional **role personas** are briefs at `ai/agents/<role>.agent.md`: a small frontmatter (`description`
required; optional `tier` cheap|mid|high|frontier and `access` read-only|full) above the brief itself. A
role is used by telling a model to act as that file - the opening instruction of a delegated subagent (run
at the brief's tier, read-only when it says so) or of a whole session; roles inherit the primary persona's
policies and are never projected into harness-specific agent formats. Briefs are project content (no rev
marker, never re-rendered); the pack README lists them (`{{AGENTS}}`);
`uv run -m solaris.tools.agents --check --dir <project>` validates them.

The ai-pack splits into a **shareable layer** (`ai/README.md` (the generated pack overview), `ai/<primary>.agent.md`, `ai/<primary>.instructions.md`, `ai/agents/*.agent.md`,
...
`{{NAME}}`, `{{TYPE}}`, `{{MODE}}`, `{{DESCRIPTION}}`, `{{FRAMEWORK_VERSION}}`, `{{DATE}}`, `{{PRIMARY}}` /
...
`ai/agents/` role briefs), `{{SKILLS}}` (trigger-invoked skills menu from the
...
mirror; see the 0.18.0 history blurb). The bundled `browserctl` plugin is the standard browser layer - CLI
...
CLI, CLI only: a gateway script that pins the CLI and installs it per project or task, a `kaggle` skill
routing agents to it and to Kaggle's own agent skill (a vendored upstream tree), and an always-on rule that
confirms every write to Kaggle first (see the 0.33.0 history blurb).
...
- `agents` - personas: `--check` validates the `ai/agents/*.agent.md` role briefs against the primary
  persona (names, frontmatter, non-empty brief); `--rename-primary <role>` moves the primary persona's two
  files, sets `agents.primary`, and re-renders the managed pack files (refusing when they are customized).

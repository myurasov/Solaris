...
**ai-setup** (`projects/<slug>/ai/`) that also works when opened on its own. Employer/domain-specific ways
...
`tasks/`. Full specification: [`spec/spec-v0.2.0.md`](spec/spec-v0.2.0.md).
...
- **Orchestrator** (this file) - at the Solaris root. Routes requests to skills; manages the project
  ...
  project work is handed to the project's developer agent via `develop-project`.
- **Developer** - inside a project (`projects/<slug>/ai/developer.agent.md`), with the ai-setup and every
...
- **Know the projects.** Each lives at `projects/<slug>/` with an ai-setup at `ai/` (descriptor:
...
- **Keep memory.** Framework `memory/`: `resources.md` (hosts/hardware), `credentials.md` (secrets,
  gitignored), `interactions.jsonl` (log). ai-setups never read this directory; copy needed values into a
...
- `uv run -m solaris.tools.version <current|aisetup|check|chain|set|plugin|check-plugins> [...]`
...
- **Per-file revisions** (`solaris.tools.revs`): every materialized framework/plugin file carries a rev
  integer + a rev-excluded content hash. ai-setups record a baseline in `ai/manifest.json` -> `revisions`.
  ...
  ai-setups stay in sync - not version numbers. After editing a revisioned file, `revs bump <file>` it and
  `revs ledger`.
...
Both are also baked into each project's `developer.agent.md` so a detached ai-setup keeps them.
...
- When the user teaches a durable preference about a project, update that project's
  `ai/memory/developer.instructions.md`; when it is about Solaris itself, use `self-reflect` to propose a

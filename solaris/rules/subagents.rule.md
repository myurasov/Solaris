...
`"subagents.level"` in `.memory/config.json` (framework) - `off` / `auto` / `quality` / `cost`;
key or file absent means `auto`. Aliases, accepted wherever the value is read: `med`/`q` =
`quality`; `full`/`save` = `cost`.

- **`off`**: leveled posture off; the bulk-read floor above still applies (it is never disabled).
- **`auto`** (default): derive from the resolved economy level (`token-economy.rule.md`): economy
  `off` -> `off`, `med` -> `quality`, `full` -> `cost`. One dial - a crunch tightens both.
...
verification passes whose outcome is a short verdict, research questions answerable from
docs/web/MCP. Independent tasks launch in one parallel batch. Before starting any multi-step
...
perishable layer - check it, not memory). Match the task class, then read the active posture's
column:

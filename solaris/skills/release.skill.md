...
4. **Update docs to reflect the changes.** Always:
   ...
   - **Info sync** — if any `solaris/info/*.md` changed since the last release, sync the pack-adapted
     copies in `solaris/templates/ai-pack/ai/info/` (edit + `revs bump`; "as of" dates must match — a
     test enforces it); the framework info file is the source of truth, the pack copies are what
     reaches projects.

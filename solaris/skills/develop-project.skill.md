...
summary: Hand off to a project's developer agent (plan or implement) by loading its ai-setup + plugin overlays.
---

# develop-project <!-- omit in toc -->

- [1. Locate the project](#1-locate-the-project)
- [2. Load the developer context](#2-load-the-developer-context)
- [3. Act](#3-act)

Thin shim: switch the active persona to a project's **developer** and carry out the user's request there.
...
## 2. Load the developer context

Read, in this order, and then obey them:

1. `projects/<slug>/ai/developer.agent.md` - the project's combined coder + planner + runner persona
   (includes the embedded commit + safety policies).
2. `projects/<slug>/ai/manifest.json` - name/type/mode + attached plugins.
3. `projects/<slug>/ai/spec.md` and `ai/memory/*` (`developer.instructions.md`, `resources.md`,
   `credentials.md`).
...
Follow the developer agent's workflows:
...
- **Learn:** when the user teaches a durable project preference, update
  `ai/memory/developer.instructions.md`. Append a line to `ai/memory/interactions.jsonl`.

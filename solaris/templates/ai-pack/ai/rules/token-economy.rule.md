...
Governs how much enters the main context and how fast it is re-sent, for all agent work on this
...
- **Prefix stability.** Time-varying fields (timestamps, counters) go at the END of always-loaded
  files, never the top; no timestamps in `AGENTS.md`, agent files, or rules. Edit an always-on
  file once per session with the change drafted fully first - never N incremental revisions of
  the same file.
...
| 3 | Prior-art check | Grep the repo for the symptom / issue id / function name before any fix | Required before any fix or new artifact; named in the report |
| 4 | Schema learning | One sibling example (module, test, config) over doc reads | Sibling example only; docs only when no example exists |
...
unique log names in a scratch dir outside the repo (session scratchpad or /tmp); exempt when the
output IS the deliverable; if a sandbox makes the redirect cost an escalation, bounded inline
output is cheaper.

## Pacing

Keep round-trips/min <= budget / current context tokens. Budget: `"economy.tokens_per_minute"`
(same config files as the level), an integer or k/m shorthand (`700k`, `1m`); absent = 1m. At 1m
...
Never save tokens by: skipping verification the safety policy requires (same-turn checks of
action claims); truncation-reading a rule, skill, or artifact you are about to modify or condense
(wording passes read the whole thing); skipping the interaction log or the `context.md` save
points; weakening confirmation, secrets, or memory duties. Token savings are a tiebreaker among
correct approaches, never a reason to degrade correctness.
...
Survey caps: ~30-35 lines/file covers most file summaries at ~57% of full-read cost; a 15-line
cap covered none and forced re-reads. Counts vs content for a yes/no check: ~2,000x cheaper (a

...
Always-on posture for all agent work on this project: **delegate self-contained work to subagents;
keep the main context lean.** The main session's job is orchestration and judgment; subagents do
the legwork. Delegation also lets cheaper/faster models carry well-scoped tasks, cutting cost and
latency. Standalone-first: needs nothing beyond this pack.

## Level Switch

`"subagents.level"` - read `ai/defaults.json` (committed team default), then `ai/.memory/config.json`
(private per-machine override; wins per key). Values `off` / `med` / `full`; absent everywhere
means `med`.
...
team-wide -> `ai/defaults.json`, this machine only -> `ai/.memory/config.json`.
...
is a short verdict (does it build, do tests pass, do links resolve), research questions answerable
from docs/web/MCP. Independent tasks launch in one parallel batch. Before starting any multi-step
...
5. **Active modes restated** - subagents do not see this pack's always-on rules; restate any
   active mode that shapes the deliverable (YAGNI, requested word counts, output conventions).

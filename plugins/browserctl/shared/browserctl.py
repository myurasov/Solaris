...
Replaces the Playwright MCP as the project's browser layer. Playwright stays
...
- A missing profile is created **clean** (empty user-data dir) on first
  ...
  init, or implicitly on first ``launch``).
...
def main() -> int:
    ...
    common.add_argument("--project",
                        help="project namespace (default: auto-derived from "
                             "the nearest ai-pack manifest / git root / CWD)")
